...
def izhikevich_neuron_sim(C, vr, vt, k, a, b, c, d, vpeak, T, I_inj, dt = 0.1):
    ...
    for i in range(n - 1):
        # Update voltage and recovery variables
        voltage[i+1] = voltage[i] + dt * (k * (voltage[i] - vr) * (voltage[i] - vt) - recovery[i] + I[i]) / C     
        recovery[i+1] = recovery[i] + dt * a * (b * (voltage[i] - vr) - recovery[i])                              
...
def plot_phase_diagram(C, vr, vt, k, a, b, I, v_trace = None, u_trace = None):
    ...
    if v_trace is not None and u_trace is not None:
        plt.plot(v_trace, u_trace, 'k--', lw = 2, alpha = 0.7, label = 'Simulation')
